...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ navigate }) => {
  ...
  const focusRingClasses = "focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E96306] focus-visible:ring-offset-2 dark:focus-visible:ring-offset-[#121212]";
  ...
  const Toggle: React.FC<{ label: string; checked: boolean; onChange: (checked: boolean) => void }> = ({ label, checked, onChange }) => (
      <div className="flex justify-between items-center py-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <button onClick={() => onChange(!checked)} className={`w-14 h-8 rounded-full p-1 transition-colors duration-300 ${focusRingClasses} ${checked ? 'bg-[#E96306]' : 'bg-gray-300 dark:bg-gray-600'}`}>
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 flex-grow flex flex-col animate-fade-in">
      <header className="flex justify-between items-center mb-6">
        <button onClick={() => navigate('home')} className={`p-2 rounded-full hover:bg-gray-200 dark:hover:bg-[#2A2A2A] ${focusRingClasses}`}>
      ...
      <div className="flex-grow overflow-y-auto">
        {/* Statistics Section */}
        {stats && (
          <div className="mb-8 p-4 bg-gray-100 dark:bg-[#2A2A2A] rounded-lg border border-transparent dark:border-gray-700">
        ...
        <div className="space-y-4">
          <div className="py-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <div className="flex justify-around mt-2 p-1 bg-gray-200 dark:bg-[#1A1A1A] rounded-lg">
               {(['light', 'dark', 'system'] as AppTheme[]).map(theme => (
                   <button key={theme} onClick={() => handleThemeChange(theme)} className={`w-full py-2 rounded-md font-semibold transition-colors ${focusRingClasses} ${settings.theme === theme ? 'bg-[#E96306] text-white' : 'hover:bg-gray-300 dark:hover:bg-[#2A2A2A]'}`}>{t(theme)}</button>
               ))}
            </div>
          </div>
          ...
          <div className="flex justify-between items-center py-4 border-b border-gray-200 dark:border-gray-700">
              ...
              <select id="language-select" value={settings.language} onChange={handleLanguageChange} className={`bg-gray-200 dark:bg-[#2A2A2A] border border-gray-300 dark:border-gray-600 rounded-md p-2 ${focusRingClasses}`}>
          ...
          <div className="py-4 border-b border-gray-200 dark:border-gray-700">
              ...
              <input id="word-length-slider" type="range" min={MIN_WORD_LENGTH} max={MAX_WORD_LENGTH} value={settings.wordLength} onChange={handleWordLengthChange} className={`w-full h-2 bg-gray-300 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-[#E96306] mt-2 ${focusRingClasses}`} />
          </div>
